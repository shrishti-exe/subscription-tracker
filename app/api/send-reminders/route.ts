...
// It can also be called manually from the reminders page.
// Authorization: either Vercel cron secret or authenticated Supabase user.
export async function POST(req: NextRequest) {
  ...
  // Get all active subscriptions with user emails via team membership
  const { data: subscriptions, error } = await supabase
    .from("subscriptions")
    .select(`
      id, name, amount, billing_cycle, start_date, category, status,
      team_id,
      teams (
        name,
        team_members (
  ...
  // Default advance notice: 3 days (could be per-user preference)
  const ADVANCE_DAYS = 3;
  const alertDate = new Date(today.getTime() + ADVANCE_DAYS * 24 * 60 * 60 * 1000);

  // Find subscriptions renewing within the advance window
  ...
  // Get user emails for team members of affected subscriptions
  const userIds = new Set<string>();
  for (const sub of upcoming) {
    const team = sub.teams as unknown as { team_members: { user_id: string }[] } | null;
    team?.team_members?.forEach((m) => userIds.add(m.user_id));
  }

  // Fetch emails from auth.users — requires service role key in production
  // For now we send to the triggering user's email
  const { data: { user } } = await supabase.auth.getUser();
  if (!user?.email) {
    return NextResponse.json({ error: "No user email found" }, { status: 400 });
  }

  const emailHtml = buildEmailHtml(upcoming, user.email);
  ...
  return NextResponse.json({ sent: 1, emailId: emailData?.id, renewals: upcoming.length });
}
      ...
      <div style="max-width:600px;margin:40px auto;background:#fff;border-radius:24px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <div style="padding:32px 40px;">
          ...
          <div style="background:#f0fdfd;border-radius:12px;padding:16px 20px;display:flex;justify-content:space-between;align-items:center;margin-bottom:32px;">
            ...
            <span style="color:#006872;font-size:20px;font-weight:800;">$${totalAmount.toFixed(2)}</span>
          </div>

          <!-- CTA -->
          <div style="text-align:center;">
            <a href="https://subscription-tracker.vercel.app/reminders"
        ...
        <div style="padding:24px 40px;border-top:1px solid #f0f0f0;">
          <p style="color:#9ca3af;font-size:12px;margin:0;text-align:center;">
            Sent to ${email} · <a href="https://subscription-tracker.vercel.app/settings" style="color:#006872;">Manage notification preferences</a>
          </p>
        </div>
      </div>
    </body>
